...
use App\admin_inspektorat_pemkot as user_pemkot;

use App\Instance as instansi;

use App\HakAksesInpektoratPemkot as hak_akses;
class HakAksesInpektoratPemkot extends Controller
{
    //
    ...
    {
        $data=[
            'data' => user_pemkot::find($id)
        ];
        return view('content_inspektorat_pemkot.page_hak_akses.index',$data);
    }

    public function get_hak_akses($id)
    {
        $userinstasi = user_pemkot::find($id);
        $dataInstansi = instansi::where('province_id', $userinstasi->province_id)->where('district_id', $userinstasi->distric_id)->where(function ($query){
        ...
        foreach ($dataInstansi as $value)
        {
            ...
            $hak_akses = hak_akses::where('admin_inspektorat_pemkot', $id)->where('instansi_id', $value->id)->count('id');
    ...
    public function tambah_hak_akses(Request $req, $id){
        ...
        $hak_akses->admin_inspektorat_pemkot = $id_inspektorat;
    ...
    public function hapus_hak_akses(Request $req, $id){
        ...
        $hak_akses =  hak_akses::where('instansi_id',$req->instansi)->where('admin_inspektorat_pemkot', $id)->first();
        if($hak_akses->delete()){
            $infor = [
                'info'=> 'success delete',
                'status'=> true
