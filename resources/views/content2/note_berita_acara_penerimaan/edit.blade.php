        ...
        <section class="content">
            ...
            <div class="col-md-12">
                <div class="box">
                    ...
                    <div class="box-body">
                        <form action="{{ url('edit_berita_acara_penerimaan_barang_jasa/'.$data->id.'/update') }}" class="form-horizontal" method="post" enctype="multipart/form-data">
                                ...
                                <div class="col-md-9">
                                    <select class="form-control select2 select2-hidden-accessible" required style="width: 100%;" tabindex="-1" aria-hidden="true"  name="id_berita_acara_HP">
                                        ...
                                        @foreach($data_surat_hasil_penerimaan as $vaitem)
                                            <option value="{{ $vaitem->id }}" @if($vaitem->id== $data->id_berita_acara_HP) selected @endif> {{ $vaitem->nomor_berita_acara}} </option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="form-group">
                                ...
                                <div class="col-md-9">
                                    <input type="text" class="form-control" required name="nomor_berita_acara" value="{{ $data->nomor_berita_acara }}">
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="col-md-2 control-label" for="username">Nomor Surat Keputusan</label>
                                <div class="col-md-9">
                                    <input type="text" class="form-control" required name="nomor_surat_keputusan" value="{{ $data->nomor_surat_keputusan }}">
                                </div>
                            </div>
                           <div class="form-group">
                                <label class="col-md-2 control-label" for="username">Tanggal Surat Keputusan</label>
                                <div class="col-md-9">
                                    <input type="text" id="datepicker2" required class="form-control" name="tgl_surat_keputusan" value="{{ date('d-m-Y', strtotime($data->tgl_surat_keputusan)) }}" data-inputmask="'alias': 'dd-mm-yyyy'" data-mask>
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="col-md-2 control-label" for="username">Tanggal Berita Acara</label>
                                <div class="col-md-9">
                                    <input type="text" id="datepicker1" required class="form-control" name="tgl_berita_acara" value="{{ date('d-m-Y', strtotime($data->tgl_berita_acara)) }}" data-inputmask="'alias': 'dd-mm-yyyy'" data-mask>
                                </div>
                            </div>
