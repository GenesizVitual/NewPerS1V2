    ...
    <div class="content-wrapper">
        ...
        <div class="col-md-12" style="padding-bottom: 1%">
            <section class="content-header">
                <h1>
                   Halaman Mutasi Tambah Kurang Persediaan
                </h1>
            </section>
        </div>
                ...
                <div class="col-md-12">
                    <div class="box box-primary">
                        <div class="box-header with-border">
                            <h3 class="box-title"> Laporan Mutasi Tambah Kurang Persediaan</h3>
                        </div>
                        <!-- /.box-header -->
                        <!-- form start {{ url('print_laporan_perjenis_barang') }} -->

                            <div class="box-body">
                            <button type="button" onclick="show_report_setting()" class="btn btn-primary">Print</button>
                                <p></p>
                                <p></p>
                                <table class="table table-bordered table-striped dataTable">
                                    ...
                                    <tr>
                                        <th>No</th>
                                        <th>Keterangan</th>
                                        <th>Persediaan Awal <br> Di Neraca <br> (Per 31 Desember {{ $tahun_anggaran-1}} ) </th>
                                        <th>Belanja Barang dan Jasa <br> (Khusus Persediaan) <br> Di LRA Selama Tahun {{ $tahun_anggaran }}</th>
                                        <th>Persediaan yang <br> digunakan selama tahun <br> {{ $tahun_anggaran }}</th>
                                        <th>Persediaan Akhir <br> Di Neraca <br> (per 31 Desember {{ $tahun_anggaran }})</th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                    ...
                                    <tfoot>
                                        <tr>
                                            <td></td>
                                            <th >Total</th>
                                            <th >{{ number_format($total_neraca_tahun_lalu,2,',','.') }}</th>
                                            <th >{{ number_format($total_persediaan_di_lra,2,',','.') }}</th>
                                            <th >{{ number_format($total_pengeluaran,2,',','.') }}</th>
                                            <th >{{ number_format($total_pesediaan_akhir_tahun,2,',','.') }}</th>
                                        </tr>
                                    </tfoot>
                                </table>
                            </div>
        ...
        $(document).ready(function () {
            ...
            table_laporan = $('#example3').DataTable({
             ...
             $('#proses_print').click(function (e) {
                e.preventDefault();
                $.ajax({
                    url: '{{ url('print-laporan-bpk-baru') }}',
                    type: 'post',
                    data : {
            ...
            show_report_setting = function(){
                 $('#modal-report-setting').modal('show');
            }
