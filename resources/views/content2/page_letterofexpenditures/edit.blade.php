    ...
    <div class="content-wrapper">
        ...
        <div class="col-md-12" style="padding-bottom: 1%">
            <section class="content-header">
                <h1>
                    Halaman Buat Surat Pengeluaran
                </h1>
            </section>
        </div>
                ...
                <div class="col-md-12">
                    <div class="box">
                        <div class="box-header with-border">
                            <h3 class="box-title">Tabel Buat Surat Pengeluaran</h3>
                        </div>
                        <form action="{{ url('UpdateletterofExpenditure/'.$data_surat->id) }}" method="post">
                            {{ csrf_field() }}
                            ...
                            <div class="box-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" id="datepicker1" class="form-control" value="{{ date('d-m-Y', strtotime($data_surat->tgl_surat_keluar)) }}" name="tgl_surat_permintaan" data-inputmask="'alias': 'dd-mm-yyyy'" data-mask>
                                </div>
                                <div class="form-group">
                                    <label for="exampleInputEmail1">Perihal Surat</label>
                                    <textarea class="form-control" name="prihal">{{ $data_surat->prihal }}</textarea>
                                </div>
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2 select2-hidden-accessible" style="width: 100%;" tabindex="-1" aria-hidden="true" name="ditujukan">
                                        ...
                                        @foreach($ditujukan as $ditujukan)
                                            <option value="{{ $ditujukan->id }}" @if($ditujukan->id==$data_surat->ditujukan) selected @endif>{{ $ditujukan->nama_berwenang}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" id="datepicker2" value="{{ date('d-m-Y', strtotime($data_surat->tgl_cetak)) }}" class="form-control" name="tgl_cetak" data-inputmask="'alias': 'dd-mm-yyyy'" data-mask>
                                </div>
                            </div>
    ...
    <script>
        ...
        $(document).ready(function(){
            ...
            $('[name="bidang"]').change(function(){
                $.ajax({
                    url:'{{ url('expenduresa_bidang') }}/'+$(this).val(),
                    dataType: 'json',
                    data:{
                        '_token' : '{{ csrf_token() }}'
                    }
